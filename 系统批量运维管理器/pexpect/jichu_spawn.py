...
#运行ls显示/tmp目录内容命令
child = 系统批量运维管理器.pexpect.spawn('ls -latr /tmp')
#内容输出到屏幕
...
#支持三个元字符（重定向>，管道|，通配符*
child = 系统批量运维管理器.pexpect.spawn('/bin/bash -c "ls -l /tmp | grep LOG > logs.txt"')
child.expect(系统批量运维管理器.pexpect.EOF)

#支持三个元字符（重定向>，管道|，通配符*，列表方式写法，推荐
shell_cmd = 'ls -l /tmp | grep LOG > logs.txt'
child = 系统批量运维管理器.pexpect.spawn('/bin/bash', ['-c', shell_cmd])
child.expect(系统批量运维管理器.pexpect.EOF)
